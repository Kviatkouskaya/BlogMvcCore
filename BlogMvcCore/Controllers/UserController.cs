...
namespace BlogMvcCore.Controllers
{
    public class UserController : Controller
    {
        ...
        {
            return login == SessionHelper.GetUserFromJson<UserDomainModel>(HttpContext.Session, "user").Login ?
                            RedirectToAction("UserPage") : View(userService.VisitUserPage(login));
        }
        public IActionResult UserPage()
        {
            var userLogin = SessionHelper.GetUserFromJson<UserDomainModel>(HttpContext.Session, "user").Login;
